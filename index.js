...
const answer1 = pokes
  .filter(pokemon => pokemon.types.includes(type)) // Filter pokemons that have the specified type
  .map(pokemon => pokemon.name); // Map the result to get only their names
...
const answer2Or = pokes
  .filter(pokemon => pokemon.types.some(pokeType => types.includes(pokeType))) // At least one matching type
  .map(pokemon => pokemon.name); // Map to names
const answer2And = pokes
  .filter(pokemon => types.every(reqType => pokemon.types.includes(reqType))) // All types must match
  .map(pokemon => pokemon.name); // Map to names
